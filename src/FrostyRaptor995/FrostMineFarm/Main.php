...
namespace FrostyRaptor995\MineFarm;
...
class Main extends PluginBase implements Listener{
    ...
    {
        //log info
        $this->getlogger()->info("§l§aMake sure you edit the plugin byyour choice");
    ...
    public function onBlockBreak(BlockBreakEvent $event){
        ...
        if($block->getLevel()->getName() === $this->config->get("levelname")){
            if($block->getId() === Block::COAL_ORE && $this->config->get("coal_ore") === true){
                ...
                $this->getScheduler()->scheduleDelayedTask(new DelayTask($this, $block), 20 * $this->config->get("glowstone_delay"));
                ...
                $block->getLevelNonNull()->setBlock($block->asVector3(), Block::get(Block::AIR));
                $this->getScheduler()->scheduleDelayedTask(new DelayTask($this, $block), 20 * $this->config->get("delay"));
            } elseif($block->getId() === Block::WOOD && $this->config->get("logs") === true){
                foreach ($event->getDrops() as $drop) {
                    $event->getPlayer()->getInventory()->addItem($drop);
                }
                $event->setDrops([]);
